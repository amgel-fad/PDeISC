document.addEventListener('DOMContentLoaded', () => {
    ...
    // ---------- Componente 6: Contador de Hijos (NUEVO) ----------
    document.getElementById('count-children').addEventListener('click', () => {
        const container = document.getElementById('child-container');
        // children devuelve solo elementos HTML (ignora nodos de texto)
        const hijos = container.children;
        const cantidad = hijos.length;
        document.getElementById('children-result').textContent = `El contenedor tiene ${cantidad} hijo(s).`;
    });
